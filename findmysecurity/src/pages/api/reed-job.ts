...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const endpoint = `https://api.adzuna.com/v1/api/jobs/gb/search/1?app_id=${app_id}&app_key=${app_key}&results_per_page=20&what=Security ${keyword}&where=${location}&salary_min=${minSalary}`;
