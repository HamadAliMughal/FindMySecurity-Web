...
const ServicesSection = ({ services, id }: { services: any; id: any }) => {
  ...
  const transformBackendData = (profileData: any) => {
    const selectedRoles = (profileData?.securityServicesOfferings || []).map((role: string) => {
      const category = professionalsList.find((cat) =>
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        const response = await axios.get(`${API_URL}/profile/individual/${id}`, {
          headers: {
        ...
        const profileData = response.data?.profileData || response.data?.profile || {};
    ...
    // Initialize with services prop if available
    if (services?.securityServicesOfferings || services?.otherService) {
      const transformedData = transformBackendData(services);
      setFormData(transformedData);
      setUpdatedData(transformedData);
      setIsLoading(false);
    } else {
